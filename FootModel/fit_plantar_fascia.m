...
% data = importdata(fullfile(pathRepo,'Figures',['Wright_Rennels_1964.csv']));
% data = data.data;
% leg(end+1)=plot(data(:,1),data(:,2),'o','Color',[1,1,1]*0.2,'LineWidth',1,'DisplayName','Data Wright and Rennels, 1964 (4 specimen)');
% plot(data(:,3),data(:,4),'o','Color',[1,1,1]*0.2,'LineWidth',1)
% plot(data(:,5),data(:,6),'o','Color',[1,1,1]*0.2,'LineWidth',1)
% plot(data(:,7),data(:,8),'o','Color',[1,1,1]*0.2,'LineWidth',1)
...
for i=["A","B","C"]
    ...
    pl_i=plot((lambda-1)*100,sigma,'-','LineWidth',2,'Color',[1,1,1]*0.5,'DisplayName',['Data Fessel et al., 2014 (3 specimen)']);
    ...
    modelfun = @(coeff,x) f_sigma(0,coeff(1),coeff(2),x);
...
%     k = coeff_sol(2)
    ...
    leg(end+1)=plot((lambda_fig-1)*100,f_fittedCurve(lambda_fig),...
        '-.','LineWidth',2, 'DisplayName',...
        sprintf("Model Natali et al, fitted on specimen %c from Fessel et al. (RMSE = %.2f MPa)",i,mdl.RMSE) );
...
%     'DisplayName','Model Natali et al, fitted on specimen C from Fessel et al.');
