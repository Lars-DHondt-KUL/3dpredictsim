...
%
fig2 = figure();
fig2.Position = [269 136 1200/2 600/2];
tl2 = tiledlayout(2,3);
...
%                 ylb.Position(1) = -27;
    ...
    %% powers
    for i=1:length(joints_ref)
        nexttile(i+2*length(joints_ref))
        % plot reference data
        if i_res==1 && i<5
...
%         nexttile(i+3*length(joints_ref))
%         % plot reference data
%         if i_res==1
...
%% heel pad power
    ...
    hold on
    plot(x,P_HC_heel,'linewidth',line_linewidth,'Color',CsV{i_res},...
        'LineStyle',mrk{rem(i_res-1,length(mrk))+1},'DisplayName',' ');
    hold on
    px=xline(x_to,'Color',CsV{i_res},'linewidth',line_linewidth/2,...
                'LineStyle',mrk{rem(i_res-1,length(mrk))+1});
