...
FigRepo = 'C:\Users\u0150099\OneDrive - KU Leuven\masterproeven\2024-2025 bilevel optimisation';
...
LegNames = {'Predictive simulation'};



joints_sim = {'hip_flexion_r','hip_adduction_r','knee_angle_r','ankle_angle_r','mtp_angle_r'};
joints_ref = {'hip_flexion','hip_adduction','knee_angle','ankle_angle','mtp_angle'};
joints_tit = {'Hip flexion','Hip adduction','Knee extension','Ankle dorsiflexion','MTP extension'};


muscles_sim = {'glut_med1_r','bifemlh_r','vas_med_r','med_gas_r','soleus_r'};
muscles_ref = {'Gluteus-medialis','Hamstrings-lateralis','Vastus-medialis','Gastrocnemius-medialis','Soleus'};
muscles_title = {'Gluteus medialis','Biceps femoris long head','Vastus medialis','Gastrocnemius medialis','Soleus'};
m_scale = [14.8, 8.4, 9.66, 3.09, 3.77];
...
% %                 ylb.Position(1) = -28;
% %             end
%             ylabel('Moment (Nm/kg)','Fontsize',label_fontsize);
%             axis tight
%             yl = get(gca, 'ylim');
%             ylim([yl(1)-0.1*norm(yl),yl(2)+0.1*norm(yl)])
%             xlim([0,100])
...
%             uistack(px,"bottom");
            ...
            max(R.a(:,idx_jsim)) / max(Data.lowEMG_mean(:,imus))
...
% 
% str = '(c)';
% annotation(gcf,'textbox',[0.40,0.55,0.05,0.05],'String',str,'EdgeColor','none','FontSize',14);
% 
% str = '(d)';
% annotation(gcf,'textbox',[0.58,0.55,0.05,0.05],'String',str,'EdgeColor','none','FontSize',14);
...
exportgraphics(fig2,fullfile(FigRepo,'figure_validation_gait.tiff'),'Resolution',300);
