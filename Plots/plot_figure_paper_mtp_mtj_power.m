...
%
fig2 = figure();
fig2.Position = [269 136 1200/2 500];
tl2 = tiledlayout(2,3);
...
for i=2:3
    nexttile(i)
    hold on
    lgp(1) = plot(x,P_mtp(idx_stance),'linewidth',line_linewidth,'Color',CsV{i_res},...
...
for i=1:6
    ...
    hold on
    Pij = pos_P_all{i};
    p1=plot(x,Pij(idx_stance),'linewidth',line_linewidth,'Color',CsV{i_res+1},...
...
pos_P_all = {P_mtj_PF, P_mtp_PF, P_PF,...
           P_mtj_PIM, P_mtp_PIM, P_PIM};
for i=1:6
    nexttile(rem(i-1,3)+1+3);
    i_res = floor((i-1)/3)+1;
    hold on
    Pij = pos_P_all{i};
    p1=plot(x,Pij(idx_stance),'linewidth',line_linewidth,'Color',CsV{i_res+3},...
        'LineStyle',mrk{rem(i_res-1,length(mrk))+1+3},'DisplayName',' ');
    hold on
...
%%
leg = {'Total joint power (nominal 2-segment foot model)',...
    'Total joint power (nominal 3-segment foot model)',...
    'Contribution of plantar fascia and plantar intrinsic muscle to joint power',...
    'Contribution of plantar fascia to joint power',...
    'Contribution of plantar intrinsic muscle to joint power'
    };
...
str = '(a)';
annotation(gcf,'textbox',[0.02,0.94,0.05,0.05],'String',str,'EdgeColor','none','FontSize',14);

str = '(b)';
annotation(gcf,'textbox',[0.02,0.61,0.05,0.05],'String',str,'EdgeColor','none','FontSize',14);
