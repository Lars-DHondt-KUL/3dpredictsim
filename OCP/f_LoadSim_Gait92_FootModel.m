...
try
    ...
    f_FiberLength_TendonForce_tendon = Function.load(fullfile(PathDefaultFunc,'f_FiberLength'));
    f_FiberVelocity_TendonForce_tendon = Function.load(fullfile(PathDefaultFunc,'f_FiberVelocity'));
end
...
if mtj
    Tau_pass_opt_inv = [jointi.hip_flex.r,jointi.hip_flex.l,...
        jointi.hip_add.r,jointi.hip_add.l,...
        jointi.hip_rot.r, jointi.hip_rot.l,...
...
Tau_pass_opt_GC(1:N-IC1i_s+1,:) = Tau_passk_opt_all(IC1i_s:end,:);
Tau_pass_opt_GC(N-IC1i_s+2:N-IC1i_s+1+N,:) = Tau_passk_opt_all(1:end,Tau_pass_opt_inv);
Tau_pass_opt_GC(N-IC1i_s+2:N-IC1i_s+1+N,Tau_pass_opt_opp) = -Tau_passk_opt_all(1:end,Tau_pass_opt_opp);
Tau_pass_opt_GC(N-IC1i_s+2+N:2*N,:) = Tau_passk_opt_all(1:IC1i_s-1,:);
% If the first heel strike was on the left foot then we invert so that
% we always start with the right foot, for analysis purpose
if strcmp(HS1,'l')
